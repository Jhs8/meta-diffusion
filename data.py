...
class metaDataset(Dataset):
    def __init__(self, data_dir,split, transform=None):
        ...
        sampled_points = np.linspace(0,300,26,dtype=int)
        if split == 'train':
            self.img_labels = np.concatenate((mat['real'][:,sampled_points],mat['imag'][:,sampled_points],mat['parameter']),axis=1)[:164883].astype(np.float32)
            self.img = mat['pattern'].transpose(2,0,1)[:164883,:32,:32].astype(np.float32)
            self.transform = transform
        elif split == 'val':
            self.img_labels = np.concatenate((mat['real'][:,sampled_points],mat['imag'][:,sampled_points],mat['parameter']),axis=1)[164883:].astype(np.float32)
            self.img = mat['pattern'].transpose(2,0,1)[164883:,:32,:32].astype(np.float32)
...
def get_dataset(name, data_dir, metadata):
    ...
    if name == "mnist":
        ...
        train_set = metaDataset(data_dir,'train',transform_train)
