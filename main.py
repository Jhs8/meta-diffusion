...
from data import get_metadata, get_dataset, fix_legacy_dict,metaDataset
import unets
...
class GuassianDiffusion:
    ...
    def __init__(self, timesteps=1000, device="cuda:4"):
...
class loss_logger:
    ...
    def log(self, v, display=False):
        ...
        if display:
            ...
            print(
                f"{formatted_time}\t"+f"Steps: {len(self.loss)}/{self.max_steps} \t loss (ema): {self.ema_loss:.3f} "
    ...
    def saveloss(self):
        np.save('loss.npy',np.array(self.loss))
...
def train_one_epoch(
    ...
    for step, (images, labels) in enumerate(dataloader):
        ...
        images, labels = (
            ...
            labels.to(args.device) if args.class_cond!=-2 else None,
        )
        ...
        if args.local_rank == 0:
            ...
            logger.log(loss.item(), display=(step == 0))
            logger.saveloss()
...
def sample_N_images(
    ...
    with tqdm(total=math.ceil(N / (args.batch_size * num_processes))) as pbar:
        while num_samples < N:
            i = num_samples // (batch_size * num_processes)
            if xT is None:
                xT = (
            ...
            if args.class_cond!=-2:
                if args.class_cond==-1:
                    index = torch.randint(0,len(args.y), (batch_size,))
                    y=args.y[index]
                    y = torch.from_numpy(y).to(args.device)
                else:
                    y = np.multiply(args.y[args.class_cond],np.ones((batch_size,55))).astype(np.float32)
                    y = torch.from_numpy(y).to(args.device)
    ...
    sample_imgs = np.zeros((N,64,64,1))
    sample_imgs[:,:32,:32] = samples
    sample_imgs[:,32:,:32] = samples[:,::-1,:]
    sample_imgs[:,:32,32:] = samples[:,:,::-1]
    sample_imgs[:,32:,32:] = samples[:,::-1,::-1]
    return (sample_imgs, np.concatenate(labels)[:N] if args.class_cond!=-2 else None)
...
def main():
    ...
    model = unets.__dict__[args.arch](
        ...
        num_classes=metadata.num_classes if args.class_cond!=-2 else None,
    ).to(args.device)
    if args.local_rank == 0:
        print(
    ...
    val_set = metaDataset(args.data_dir,'val',transform=transform)
    args.y = val_set.img_labels
    # sampling
    if args.sampling_only:
        ...
        np.savez(
            os.path.join(
                args.save_dir,
                f"{args.arch}_{args.dataset}-{args.sampling_steps}-sampling_steps-{len(sampled_images)}_images-class_condn_{args.class_cond}_epoch_{args.pretrained_ckpt[-6:-3]}.npz",
            ),
            sampled_images,
            labels,
        )
        ...
        if not epoch % 2:
            ...
            if args.local_rank == 0:
                np.savez(
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}_{epoch}.npz",
                    ),
                    sampled_images,
                    labels,
                )
            if args.local_rank == 0:
                torch.save(
                    ...
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{args.dataset}-epoch_{args.epochs}-timesteps_{args.diffusion_steps}-class_condn_{args.class_cond}_{epoch}.pt",
                    ),
                )
                torch.save(
                    args.ema_dict,
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{args.dataset}-epoch_{args.epochs}-timesteps_{args.diffusion_steps}-class_condn_{args.class_cond}_ema_{args.ema_w}_{epoch}.pt",
                    ),
                )
